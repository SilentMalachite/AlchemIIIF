...
| `extracted_images` | Figure Assets | `image_path`, `geometry` (crop data) | `:string`, `:map` (JSONB) |
| `iiif_manifests` | Manifest Entities | `identifier`, `metadata` | `:string`, `:map` (JSONB) |

## 4. IIIF Server Implementation (Delivery)
...
## 5. "Manual Inspector" Workflow (Ingestion Pipeline)
...
### 5.2 Accessibility Feature: "Nudge" Controls
...
### 6.1 JS Hook (Manual Crop with Nudge Support)
...
## 8. Implementation Instructions for AI Agents (Antigravity)
