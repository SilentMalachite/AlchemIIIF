defmodule AlchemIiifWeb.SearchLiveTest do
  ...
  describe "search イベント" do
    test "テキスト検索が実行される", %{conn: conn} do
      ...
      assert html =~ "fig-search-test" or html =~ "件の図版"
    end
  ...
  describe "toggle_filter イベント" do
    test "フィルターがトグルされる", %{conn: conn} do
      ...
      # フィルターをクリック
      html = render_click(view, "toggle_filter", %{"type" => "site", "value" => "テスト遺跡A"})
    ...
    test "同じフィルターを再クリックでクリアされる", %{conn: conn} do
      ...
      # フィルターを有効化
      render_click(view, "toggle_filter", %{"type" => "site", "value" => "テスト遺跡B"})

      # もう一度クリックしてクリア
      html = render_click(view, "toggle_filter", %{"type" => "site", "value" => "テスト遺跡B"})
      assert html =~ "件の図版" or html =~ "結果なし"
    end
  end
