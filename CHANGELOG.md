...
### 🔍 ギャラリー検索の改善
- **「もっと見る」ボタン方式のページネーションを追加 (`gallery_live.ex`, `search.ex`)**
  - 検索結果を 10 件ずつ表示し、ボタンクリックで次の 10 件を下部に追加表示。
  - `Search.search_published_images/3` に `limit` / `offset` オプションを追加（デフォルト `limit: 10`）。
  - 検索テキスト変更・フィルター変更・クリア時はページを 1 にリセットして先頭 10 件を再取得。
  - 全件表示後はボタンを自動非表示。残件数をボタンラベルに表示（例: 「もっと見る（残り 5 件）」）。
- **フィルター候補を公開済み画像由来の値だけに絞り込み (`search.ex`)**
  - `Search.list_filter_options/1` に `published_only` オプションを追加。
  - Gallery では `published` かつ PTIF 生成済みの画像由来の値のみフィルター候補に表示するよう変更。
...
- **Claude Code 向けプロジェクトガイド (`CLAUDE.md`) を新設**
...
- **IIIF モジュールの命名規則の統一**
  - モジュールロードエラーを解消するため、ケーシングを `AlchemIiif.IIIF` から `AlchemIiif.Iiif` に統一。
  - 関連する各ファイル（`AlchemIiif.Iiif.PtiffGenerator` など）の `defmodule` および参照を修正し、コンパイルエラー・警告を除去。
...
- **`AlchemIiif.IIIF.PtiffGenerator` モジュールの新設**
