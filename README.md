...
| **3. クロップ** | ✂️ | オリジナルの JS Hook を使用。**ダブルクリック（またはダブルタップ）**で範囲を確定・保存します |
...
| ステージ | 説明 |
|:---:|:---|
| **Lab (内部)** | PDF のアップロード・クロップ・ラベリングを行う内部ワークスペース |
| **承認** | 管理者が `/admin/review` で内容を確認、承認、または削除を行います |
| **Gallery (公開)** | 承認されたアセットを表示。高解像度クロップ画像のダウンロードが可能です |
