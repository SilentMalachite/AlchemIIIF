...
| **3. クロップ** | ✂️ | オリジナルの JS Hook を使って図版の範囲を指定します。方向ボタンで微調整も可能です |
| **4. ラベリング** | 🏷️ | キャプション・ラベル・遺跡名・時代・遺物種別などのメタデータを入力します |
| **5. レビュー提出** | ✅ | 入力内容を確認し、PTIF 生成・IIIF Manifest 登録を実行します |
...
| **承認** | 管理者が `/admin/review` で内容を確認し、公開を承認します |
| **Gallery (公開)** | 承認されたアセットのみが表示される公開ギャラリー（新潟インディゴ＆ハーベストゴールドテーマ） |
...
- **ギャラリーテーマ**: 「新潟インディゴ＆ハーベストゴールド」— Deep Sea Indigo (#1A2C42) 背景 + Harvest Gold (#E6B422) アクセント、Mist Grey (#E0E0E0) テキストでコントラスト比約 10.4:1（WCAG AAA 準拠）
